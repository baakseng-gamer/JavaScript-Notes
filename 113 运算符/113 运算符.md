...
2. 第二个操作数是对象，返回求值结果为true的第一个操作数对象。
...
```
    let a = [myName = "baakseng", age = 30];
    let b = [msg = "ABC"];
    console.log(a || 6 == 3); //(2) ["baakseng", 30]

    console.log(6 == 3 || b); //["ABC"]
    console.log(6 > 3 || b); //true
    console.log(b || 6 > 3); //["ABC"]
    console.log(b || a); //["ABC"]
